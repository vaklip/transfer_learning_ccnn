...
The weights and bias terms of the convolutional layers are fixed and correspond
to those learned previously on the public dataset, stored in 'weights_public'. 
...
of the 10-fold cross-validation on the in-house dataset (folds are stored in
'folds_inhouse.npy). Results are saved into 'results_ccnn_class_CONVconstFULLtrain.npz' 
...
# Save data
if initmode == 1:
    np.savez("results_ccnn_class_CONVconstFULLtrain.npz", \
