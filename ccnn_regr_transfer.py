# -*- coding: utf-8 -*-
"""
Created on Wen Nov 15 13:16:24 2017

This script trains the fully connected layers of a connectome-convolutional 
neural network on the in-house dataset / NKI-RS subset to regress chronological 
age against resting-state functional connectivity matrices using 10-fold 
cross-validation (the folds are stored in 'folds_inhouse.npy' / 
'folds_NKI-RS_subset.npy'). The weights and  biases of the convolutional layers 
are constants corresponding to those learned previously on the public dataset to 
classify age category. Weights and biases of the fully connected layers are 
initialized using the values learned on the public dataset to regress chronological 
age against functional connectivity matrices. These values are stored in 
'weights_public_regr.pickle' in the root directory, which is  generated by 
'ccnn_regr_public.py'. The results are saved into 'results_ccnn_regr_transfer*.npz'.
...
# Loading weights
picklefile = "weights_public_regr.pickle"
...
for i in range(num_folds):
    ...
    # Creating train and test data for each fold    
...
# Saving data
if target_data == 1:
    np.savez("results_ccnn_regr_transfer_inhouse.npz", labels=l, predictions=p, splits=IDs)
elif target_data == 2:
    np.savez("results_ccnn_regr_transfer_NKI-RS_subset.npz", labels=l, predictions=p, splits=IDs)
    
# Saving weights and biases
if target_data == 1:
    pickle_file = "weights_ccnn_regr_transfer_inhouse.pickle"
elif target_data == 2:
    pickle_file = "weights_ccnn_regr_transfer_NKI-RS_subset.pickle"
